...
import {
  ...
  Actions,
  Separator,
} from '../../workspace/InfoRow/InfoRow';
import { copyText } from '../../../lib/clipboard';
import * as adminApi from '../../../api/admin';
import type { AdminLink } from '../../../api/admin';
import './AdminLinkRow.css';
...
export function AdminLinkRow({
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Xóa liên kết "${name}"? Không thể hoàn tác.`)) return;
    try {
      ...
      ? [
          {
            ...
            onSelect: () => navigate(`/admin/links/${link.id}`),
          },
        ]
      : []),
  ...
  return (
    ...
    >
      <Cell className="name-cell">
        ...
        <Sub>
          {link.owner ? (
            `Tạo bởi ${link.owner.fullName || link.owner.username || link.owner.email}`
          ) : (
            <span className="admin-link-orphaned">Không rõ chủ sở hữu</span>
          )}
        </Sub>
      </Cell>
      ...
      <Cell className="status-cell">
        <Extra>
          <Status active={link.isActive}>{link.isActive ? 'Đang hoạt động' : 'Ngừng hoạt động'}</Status>
          <Separator />
