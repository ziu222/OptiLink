...
fn vertexMain(@location(0) position: vec3<f32>, @location(1) normal: vec3<f32>,
  ...
  var local = turn(position * vec3<f32>(1.8,1.18,1.8),instance.w) * instance.z;
  ...
  out.color = pow(clamp(pigment * diffuse * (0.85 + seed * 0.2),vec3<f32>(0.0),vec3<f32>(1.0)),vec3<f32>(0.7));
  out.seed = seed;
  return out;
}
@fragment
