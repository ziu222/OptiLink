      ...
      <div className="qr-studio page-content">
        ...
        <section className="qr-studio-grid" aria-busy={loading}>
          <div className="qr-studio-library">
            ...
            {loading && <p className="qr-empty">Đang tải link…</p>}
            ...
            <div className="qr-link-list" role="listbox" aria-label="Chọn link để tạo QR">
              {links.map((link) => (
                <button key={link.id} type="button" className={`qr-link-choice${selectedId === link.id ? ' is-selected' : ''}`} onClick={() => { setSelectedId(link.id); setQr(null); }} role="option" aria-selected={selectedId === link.id}>
          ...
          <div className="qr-studio-canvas">
            ...
            <div className="qr-studio-actions">
              <button type="button" className="qr-primary-action" onClick={buildQr} disabled={!selectedLink || building}>{building ? 'Đang tạo…' : qr ? 'Làm mới QR' : 'Tạo QR Code'}</button>
